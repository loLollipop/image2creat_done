...
class PlatformRegistrar:
    ...
    def _login_and_exchange_tokens(self, email: str, password: str, mailbox: dict, index: int) -> dict:
        # 用全新的 session + device_id 走登录换 token 流程。
        # 注册成功后，原 self.session 已被 OpenAI 标记为「已完成注册」状态，
        # 再次在同一 session 上 authorize → password_verify 会返回
        # HTTP 409 invalid_state「Invalid session. Please start over.」，
        # 导致整个注册流程在最后一步全数失败。
        # 见 upstream issue basketikun/chatgpt2api#155。
        step(index, "开始独立登录换 token (fresh session)")
        login_session = create_session(config.get("proxy") or "")
        login_device_id = str(uuid.uuid4())
        try:
            login_session.cookies.set("oai-did", login_device_id, domain=".auth.openai.com")
            login_session.cookies.set("oai-did", login_device_id, domain="auth.openai.com")

            def _login_navigate_headers(referer: str) -> dict[str, str]:
                hdrs = dict(navigate_headers)
                if referer:
            ...
            def _login_json_headers(referer: str) -> dict[str, str]:
                ...
                hdrs["referer"] = referer
                hdrs["oai-device-id"] = login_device_id
                hdrs.update(_make_trace_headers())
                return hdrs
            ...
            resp, error = request_with_local_retry(login_session, "get", f"{auth_base}/api/accounts/authorize?{urlencode(params)}", headers=_login_navigate_headers(f"{platform_base}/"), allow_redirects=True, verify=False)
            if resp is None:
